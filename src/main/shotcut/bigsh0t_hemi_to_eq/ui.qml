...
Item {
    ...
    property double distortionAStart: 0
    property double distortionAMiddle: 0
    property double distortionAEnd: 0
    property double distortionBStart: 0
    property double distortionBMiddle: 0
    property double distortionBEnd: 0
    property double distortionCStart: 0
    property double distortionCMiddle: 0
    property double distortionCEnd: 0
    property double distortionRadiusStart: 0
    property double distortionRadiusMiddle: 0
    property double distortionRadiusEnd: 0

    property double vignettingAStart: 0
    property double vignettingAMiddle: 0
    property double vignettingAEnd: 0
    property double vignettingBStart: 0
    property double vignettingBMiddle: 0
    property double vignettingBEnd: 0
    property double vignettingCStart: 0
    property double vignettingCMiddle: 0
    property double vignettingCEnd: 0
    property double vignettingDStart: 0
    property double vignettingDMiddle: 0
    property double vignettingDEnd: 0
    property double vignettingRadiusStart: 0
    property double vignettingRadiusMiddle: 0
    property double vignettingRadiusEnd: 0

    property double emorH1Start: 0
    property double emorH1Middle: 0
    property double emorH1End: 0
    property double emorH2Start: 0
    property double emorH2Middle: 0
    property double emorH2End: 0
    property double emorH3Start: 0
    property double emorH3Middle: 0
    property double emorH3End: 0
    property double emorH4Start: 0
    property double emorH4Middle: 0
    property double emorH4End: 0
    property double emorH5Start: 0
    property double emorH5Middle: 0
    property double emorH5End: 0
    ...
    function updateSimpleKeyframes() {
        if (filter.animateIn > 0 || filter.animateOut > 0) {
            ...
            UPDATE_SIMPLE_KEYFRAMES(distortionA)
            UPDATE_SIMPLE_KEYFRAMES(distortionB)
            UPDATE_SIMPLE_KEYFRAMES(distortionC)
            UPDATE_SIMPLE_KEYFRAMES(distortionRadius)

            UPDATE_SIMPLE_KEYFRAMES(vignettingA)
            UPDATE_SIMPLE_KEYFRAMES(vignettingB)
            UPDATE_SIMPLE_KEYFRAMES(vignettingC)
            UPDATE_SIMPLE_KEYFRAMES(vignettingRadius)

            UPDATE_SIMPLE_KEYFRAMES(emorH1)
            UPDATE_SIMPLE_KEYFRAMES(emorH2)
            UPDATE_SIMPLE_KEYFRAMES(emorH3)
            UPDATE_SIMPLE_KEYFRAMES(emorH4)
            UPDATE_SIMPLE_KEYFRAMES(emorH5)
        }
        setControls();
        ...
        updateProperty_distortionA(null);
        updateProperty_distortionB(null);
        updateProperty_distortionC(null);
        updateProperty_distortionRadius(null);
        updateProperty_vignettingA(null);
        updateProperty_vignettingB(null);
        updateProperty_vignettingC(null);
        updateProperty_vignettingD(null);
        updateProperty_vignettingRadius(null);
        updateProperty_emorH1(null);
        updateProperty_emorH2(null);
        updateProperty_emorH3(null);
        updateProperty_emorH4(null);
        updateProperty_emorH5(null);
    ...
    function setControls() {
        ...
        SETCONTROLS(distortionA)
        SETCONTROLS(distortionB)
        SETCONTROLS(distortionC)
        SETCONTROLS(distortionRadius)

        SETCONTROLS(vignettingA)
        SETCONTROLS(vignettingB)
        SETCONTROLS(vignettingC)
        SETCONTROLS(vignettingD)
        SETCONTROLS(vignettingRadius)
    ...
    Component.onCompleted: {
        ...
        ONCOMPLETED(distortionA, 0.0)
        ONCOMPLETED(distortionB, 0.0)
        ONCOMPLETED(distortionC, 0.0)
        ONCOMPLETED(distortionRadius, 1.0)

        ONCOMPLETED(vignettingA, 0.0)
        ONCOMPLETED(vignettingB, 0.0)
        ONCOMPLETED(vignettingC, 0.0)
        ONCOMPLETED(vignettingD, 0.0)
        ONCOMPLETED(vignettingRadius, 1.0)

        ONCOMPLETED(emorH1, 0.0)
        ONCOMPLETED(emorH2, 0.0)
        ONCOMPLETED(emorH3, 0.0)
        ONCOMPLETED(emorH4, 0.0)
        ONCOMPLETED(emorH5, 0.0)
...
#define UPDATE_PHOTOMETRIC_PARAMETER(PROPERTY,NAME,DEFAULT_VALUE) $\
    function updateProperty_##PROPERTY(position) { $\
        if (blockUpdate) { $\
            return; $\
        } $\
    ...
    UPDATE_PHOTOMETRIC_PARAMETER(distortionA, "A", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(distortionB, "B", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(distortionC, "C", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(distortionRadius, "Radius", 0.0)

    UPDATE_PHOTOMETRIC_PARAMETER(vignettingA, "A", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(vignettingB, "B", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(vignettingC, "C", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(vignettingD, "D", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(vignettingRadius, "Radius", 0.0)

    UPDATE_PHOTOMETRIC_PARAMETER(emorH1, "H1", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(emorH2, "H2", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(emorH3, "H3", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(emorH4, "H4", 0.0)
    UPDATE_PHOTOMETRIC_PARAMETER(emorH5, "H5", 0.0)
    ...
    GridLayout {
        ...
        Shotcut.Preset {
            ...
            onBeforePresetLoaded: {
                ...
                filter.resetProperty('distortionA');
                filter.resetProperty('distortionB');
                filter.resetProperty('distortionC');
                filter.resetProperty('distortionRadius');

                filter.resetProperty('vignettingA');
                filter.resetProperty('vignettingB');
                filter.resetProperty('vignettingC');
                filter.resetProperty('vignettingD');
                filter.resetProperty('vignettingRadius');

                filter.resetProperty('emorH1');
                filter.resetProperty('emorH2');
                filter.resetProperty('emorH3');
                filter.resetProperty('emorH4');
                filter.resetProperty('emorH5');
            }
            onPresetSelected: {
                yawMiddle = filter.getDouble("yaw", filter.animateIn);
                if (filter.animateIn > 0)
                ...
                ONPRESETSELECTED(distortionA)
                ONPRESETSELECTED(distortionB)
                ONPRESETSELECTED(distortionC)
                ONPRESETSELECTED(distortionRadius)

                ONPRESETSELECTED(vignettingA)
                ONPRESETSELECTED(vignettingB)
                ONPRESETSELECTED(vignettingC)
                ONPRESETSELECTED(vignettingD)
                ONPRESETSELECTED(vignettingRadius)

                ONPRESETSELECTED(emorH1)
                ONPRESETSELECTED(emorH2)
                ONPRESETSELECTED(emorH3)
                ONPRESETSELECTED(emorH4)
                ONPRESETSELECTED(emorH5)
...
#define PHOTOMETRIC_PARAMETER(PROPERTY,NAME,DEFAULT_VALUE) $\
        Label {$\
            text: qsTr(NAME)$\
            Layout.alignment: Qt.AlignRight$\
        }$\
...
$\
            ...
            decimals: 4$\
            stepSize: 0.0001$\
            onValueChanged: updateProperty_##PROPERTY(getPosition())$\
        }$\
$\
        Shotcut.UndoButton {$\
            id: PROPERTY##Undo$\
$\
            onClicked: PROPERTY##Slider.value = DEFAULT_VALUE $\
        }$\
$\
        Shotcut.KeyframesButton {$\
...
$\
        ...
        PHOTOMETRIC_PARAMETER(distortionA, "A", 0.0)
        PHOTOMETRIC_PARAMETER(distortionB, "B", 0.0)
        PHOTOMETRIC_PARAMETER(distortionC, "C", 0.0)
        PHOTOMETRIC_PARAMETER(distortionRadius, "Radius", 0.0)
        ...
        PHOTOMETRIC_PARAMETER(vignettingA, "A", 0.0)
        PHOTOMETRIC_PARAMETER(vignettingB, "B", 0.0)
        PHOTOMETRIC_PARAMETER(vignettingC, "C", 0.0)
        PHOTOMETRIC_PARAMETER(vignettingD, "D", 0.0)
        PHOTOMETRIC_PARAMETER(vignettingRadius, "Radius", 0.0)
        ...
        PHOTOMETRIC_PARAMETER(emorH1, "H1", 0.0)
        PHOTOMETRIC_PARAMETER(emorH2, "H2", 0.0)
        PHOTOMETRIC_PARAMETER(emorH3, "H3", 0.0)
        PHOTOMETRIC_PARAMETER(emorH4, "H4", 0.0)
        PHOTOMETRIC_PARAMETER(emorH5, "H5", 0.0)
