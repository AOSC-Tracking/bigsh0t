...
Item {
    ...
    function updateProperty_maxStep(position) {
        if (blockUpdate)
            return;
        var value = maxStepSlider.value;
        filter.set("maxStep", value);
    }
    ...
    Component.onCompleted: {
        ...
        if (filter.isNew)
            filter.set("maxStep", 16);
        else
            maxStepValue = filter.getDouble("maxStep");
        if (filter.isNew)
    ...
    GridLayout {
        ...
        Shotcut.Preset {
            ...
            parameters: ["maxStep", "interpolation", "stabilizeYaw", "stabilizePitch", "stabilizeRoll", "smoothYaw", "smoothPitch", "smoothRoll", "timeBiasYaw", "timeBiasPitch", "timeBiasRoll"]
            Layout.columnSpan: 3
            onBeforePresetLoaded: {
                filter.resetProperty("maxStep");
        ...
        Shotcut.SliderSpinner {
            ...
            stepSize: 1
            Layout.columnSpan: 2
            onValueChanged: updateProperty_maxStep(getPosition())
        }

        Shotcut.UndoButton {
            id: maxStepUndo

            onClicked: maxStepSlider.value = 8
        }
